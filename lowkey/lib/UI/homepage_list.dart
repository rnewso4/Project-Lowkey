...
newsView() {
  return Expanded(
    child: ListView.separated(
      padding: const EdgeInsets.all(8),
      itemCount: businesses.length,
      itemBuilder:(context, index) => tilesForNews(businesses[index].getNewsTitle(), businesses[index].getLogo(), businesses[index].getNewsDescription()),
      separatorBuilder: (context, index) => const SizedBox(height: 20)
    ),
  );
}
  ...
  List<Business> businesses = <Business>[
    ...
    Business('Highland Cofee', 'Our pumpkin spice latte is back at high demand', 'lib/UI/assets/highland.png'),
    Business('Highland Coffee', 'Doja Cat will be performing live on Saturday', 'lib/UI/assets/highland.png'),
    Business('Reginelli’s Pizzeria', '\$5 off every coffee purchased before noon', 'lib/UI/assets/reg.jpeg'),
    Business('Reginelli’s Pizzeria', 'Our pumpkin spice pizza is back at high demand', 'lib/UI/assets/reg.jpeg'),
    Business('Reginelli’s Pizzeria', 'Buy two, get a third one free every weekend', 'lib/UI/assets/reg.jpeg'),
  ];
